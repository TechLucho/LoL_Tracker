...
async def list_matches(
    ...
    rows = await repo.list_recent(user_id, limit=limit, offset=offset, queue=queue)
...
async def update_match(user_id: CurrentUserId, game_id: str, payload: MatchUpdate) -> Match:
    ...
    if await repo.get_by_id(user_id, game_id) is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, f"Partida {game_id} no encontrada")

    await repo.update_details(user_id, game_id, changes)
    row = await repo.get_by_id(user_id, game_id)
    return _serialize(row)  # type: ignore[arg-type]


@router.get("/{game_id}/scout-opponent", response_model=ScoutOpponent)
async def scout_opponent(user_id: CurrentUserId, game_id: str) -> ScoutOpponent:
